...
- **`components/scanner-page-client.tsx`**, **`components/setup-outcomes-page-client.tsx`**, **`components/journal-page-client.tsx`** — N-of-N `<Link prefetch={false}>` targets to `/dashboard/signals` carry **`data-hover-prefetch="true"`** + per-row/per-setup **`useHoverPrefetch(href)`** (scanner **Open Signals**, setup-outcomes symbol links, journal signal cells) so intent warms the route without mount prefetch.
