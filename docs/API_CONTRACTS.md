...
- `POST /v1/signals/assistant/chat` — **authenticated**. STOCVEST Assistant conversational explanations. **Caller surface:** the BFF route at `app/api/stocvest/signals/assistant/chat/route.ts` proxies this; the only client-side caller is `components/assistant/stocvest-assistant.tsx`, which is mounted in `app/layout.tsx` (root layout) **gated on `getServerSession()`** — anonymous marketing visitors never see the assistant and never trigger this endpoint, so there is no unauthenticated public surface to defend. Logged-in users get the assistant on every route; **every dashboard page** publishes a whitelisted `page_context` via `usePublishAssistantContext`, and unknown keys are silently dropped server-side regardless. Request body: 
  ```jsonc
  {
    "messages": [
