...
- `GET /v1/signals/position/candidates` — **ADR-004 POS-D15 (stub)** ranked gem-candidate screen. Auth required. Query: **`tier`** (`gem`\|`strong`\|`monitor`\|`all`; default `gem`), **`limit`** (1–100, default 50), optional **`refresh=1`** (force rescan). Success: **`mode": "position"`**, **`tier`**, **`candidates[]`** (each: `symbol`, `tier`, `rank` 0–100, `composite_score`, `verdict`, `fundamentals_score`/`fundamentals_verdict`, `technical_score`/`technical_verdict`, `sector_verdict`, `data_quality`, `weakest_pillar_id`/`weakest_pillar_label`, `rs_vs_spy_6m_pct`, `signal_valid_days`, `why` (non-advisory), `pillars[]` (F1–F5 `{pillar_id,label,score,verdict,data_quality}`), `failing_gates[]`), **`count`**, **`universe_size`**, **`scan_generated_at`** (ISO UTC), **`cached`** (bool), **`disclaimer`**. Tiers/gates (**G1–G9**) and transparent **`gem_rank`** = `0.45·fundamentals + 0.20·technical + 0.15·sector + 0.10·macro + 0.10·min(F1..F5)` are deterministic (no LLM). Scan runs a curated liquid US universe (v1 lite); weekly batch + ~500 names scale after POS-D9. Degrades to `{ candidates: [], degraded: true }` on scan failure (still HTTP 200). "Gem candidate" = passed internal quality gates for **informational screening**, not a recommendation. BFF: **`GET /api/stocvest/signals/position/candidates`**.
