...
**Last reviewed:** 2026-09-13 (§4.16–4.17 personal-mode default is **conviction sleeves**, not `100 / max(N, 8)`; additive review `sizingPolicy` + per-holding `sleeve` / `sleeveLowPct` / `sleeveHighPct`; prior — §4.17 holdings[] additive **`isFundVehicle`** + review AI `audience=holder`; §4.16 holdings + §4.17 portfolio-review Lambdas + `Holdings` table + `/v1/holdings*` and `/v1/portfolio-review` API routes deployed via `terraform apply` — status flipped from "deploy pending" to "deployed 2026-09-12")
