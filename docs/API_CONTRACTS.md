...
- `GET /v1/signals/me/history` — authenticated user’s evaluated signals; query `symbol`, `days` (1–365), `limit` (1–200), optional `mode` (`day` \| `swing`) for the signal validation ledger. Response rows include core D1 fields plus optional ledger keys when present: `closed_at`, `ledger_entry_date_et`, `ledger_exit_date_et`, `entry_rationale`, `exit_reason`, `decision_state_entry`, `decision_state_exit`, `market_regime_exit`, `gate_status` (object, parsed from `gate_status_json`), `setup_type`, `exit_rule`, `max_adverse_excursion_pct`, `max_favorable_excursion_pct`, `hold_duration_minutes`, `layer_scores`, `mode`, `status`.
