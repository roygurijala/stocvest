...
**Last reviewed:** 2026-09-12 (§4.17 holdings[] additive **`isFundVehicle`** + review AI `audience=holder`; prior — §4.16 holdings + §4.17 portfolio-review Lambdas + `Holdings` table + `/v1/holdings*` and `/v1/portfolio-review` API routes deployed via `terraform apply` — status flipped from "deploy pending" to "deployed 2026-09-12")
...
- `PUT /v1/holdings/settings` — body **`{ cashBalance ≥ 0, targetPositionPct (0,100] | null, benchmarkSymbol }`** (a client-supplied `userId`/`user_id` is rejected `400`); returns the stored settings. `targetPositionPct` is the target weight per position as a % of total portfolio value (holdings-at-cost + cash); `null` means no saved target (personal mode uses a review-time equal-weight default `100 / max(N, 8)` for amounts; product mode stays directional with no suggested amounts). The default is never written unless the user saves an explicit target.
...
- `GET /v1/portfolio-review` — returns the review for the caller's portfolio. Shape (camelCase): **`generatedAt`**, **`holdings[]`**, **`totalMarketValue`**, **`investedValue`**, **`cashBalance`**, **`totalCost`**, **`unrealizedPl`**/**`unrealizedPlPct`**, **`portfolioReturnPct`**, **`concentration[]`**, **`considerAdding[]`**, **`benchmark`**, **`fullyPriced`**, **`effectiveTargetPct`** (additive — the weight used for gap math: the saved `targetPositionPct` when set, else a personal-mode equal-weight default `100 / max(N, 8)` computed at review time and **not** written back to settings), **`targetIsDefault`** (true only when that default was used), **`sizingRule`** (additive header one-liner for the already-shipped stance overlay: sell reduces even when under target; hold/neutral + caution does not add toward target; otherwise move toward the target), **`disclaimer`**. Additive cache envelope (Slice 3b): **`pending`**, **`cached`**, **`cachedAt`**, **`stale`**. A cache miss without a stored review returns **`{ pending: true }`** (HTTP 200) and does **not** run the Long-Term composite on the request path (that work exceeds the API Gateway ~29s cap). **`?refresh=1`** kicks a background recompute (async Lambda self-invoke); the client polls the plain GET until `pending` is false and `holdings` is present.
  - Each `holdings[]` row: **`symbol`**, **`quantity`**, **`averageCost`**, **`currentPrice`** (null → valued at cost, P/L null not $0), **`marketValue`**, **`unrealizedPl`**/**`unrealizedPlPct`**, **`weightPct`**, **`verdict`**, **`confidence`**, **`action`** ∈ `buy_more|hold|trim|sell|review` (+ **`actionLabel`**), **`rationale[]`**, **`overweight`**, **`suggestedAddAmount`**/**`suggestedReduceAmount`** (gap-to-target dollars when an effective target exists in personal mode: add = underweight gap cash-capped, trim = excess only, sell = full market value; null when product mode has no saved target), **`sizingReason`** (additive short sentence for why add/reduce is or is not set — sell-under-target, hold+caution no-add, trim-excess, add-gap, or at-target/no change; the hold+caution dollar is informational unfilled gap only and does **not** set `suggestedAddAmount`), **`effectiveTargetPct`** (same review-time target as the header), **`taxLotHint`** + **`longTermLots`**/**`shortTermLots`**, **`isFundVehicle`** (additive; true for ETF/ETN/fund products — F1–F5 corporate pillars do not apply), **`holderRead`** (stance/headline/actions/context), **`aiRead`** (per-holding AI Investment Read — a Claude narration via the gated `explain_position_setup_read` with **`audience=holder`**; `null` for free accounts / when no profile, so the review stays deterministic and free of LLM cost). Review AI also receives the suggested add/reduce dollars so narration cannot invent a different size.
