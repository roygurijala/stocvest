...
**Composite `news_source` (additive, ADR-001):** Swing composite responses always include **`news_source": "polygon_primary"`**. Day composite includes the same when **`STOCVEST_DAY_COMPOSITE_BENZINGA_ENABLED`** is off (default).

**Composite `benzinga_feed_health` (deprecated on swing — ADR-001 DBZ-8):** **Removed** from **`POST /v1/signals/composite/swing`** responses as of DBZ-8. Swing desk no longer calls Benzinga; clients must use **`news_source`** and the News layer row instead. Day composite may still include **`benzinga_feed_health`** when legacy Benzinga is enabled via **`STOCVEST_DAY_COMPOSITE_BENZINGA_ENABLED=1`**; omitted when Polygon-primary (default). Legacy swing clients that read **`benzinga_feed_health`** should treat its absence as “Benzinga not configured.”
