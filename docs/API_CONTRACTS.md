...
- `POST /v1/signals/position/research` — **authenticated**. ADR-004 POS-AI-4 Position "Research" tab. **Paid-gated** (`has_ai_explanations`) **and flag-gated** (`STOCVEST_POSITION_RESEARCH_ENABLED`, default OFF — ships dark). BFF: `app/api/stocvest/signals/position/research/route.ts` (transparent forward). Request body: `{ "symbol": "AAPL", "company_name"?: "Apple Inc" }`. Assembles two **external** sources in parallel — a cited Perplexity "recent developments" summary and a truncated SEC 10-K **Item 1A · Risk Factors** excerpt (`stocvest/data/edgar_10k.py`, source filing URL included). Per-user/day **soft budget** (`STOCVEST_POSITION_RESEARCH_MAX_PER_USER_PER_DAY`, default 15; Redis-backed, no-op without Redis). Response: `{ "symbol", "status": "ok"|"disabled"|"upgrade_required"|"over_budget"|"empty", "recent_developments": { "summary", "key_points"[], "sources"[{title,url}], "scored": false } | null, "risk_factors": { "excerpt", "source_url", "filing_date", "form", "truncated", "scored": false } | null, "upgrade_available", "disclaimer" }`. **Contract guarantee:** this content is **INFORMATIONAL ONLY** — every block carries `scored: false` and is **never** merged into the composite score / pillar math; the prompt forbids advice and the payload always carries a non-advisory `disclaimer`.
