...
- `GET /v1/signals/position/candidates` — **ADR-004 POS-D15** ranked gem-candidate screen. Auth required. Query: **`tier`** (`gem`\|`strong`\|`monitor`\|`all`; default `gem`), **`limit`** (1–100, default 50), optional **`refresh=1`** (kick an async compose; keep serving the last snapshot if one exists, else `{pending:true}`; live discovery pond is FMP `limit=1500` + `marketCapLowerThan` $200B, `LIVE_DISCOVERY_MAX=15`). Success: **`mode": "position"`**, **`tier`**, **`candidates[]`** (each: `symbol`, `tier`, `rank` 0–100, `composite_score`, `verdict`, `fundamentals_score`/`fundamentals_verdict`, `technical_score`/`technical_verdict`, `sector_verdict`, `data_quality`, `weakest_pillar_id`/`weakest_pillar_label`, `rs_vs_spy_6m_pct`, `signal_valid_days`, `why` (non-advisory), `pillars[]` (F1–F5 `{pillar_id,label,score,verdict,data_quality}`), `failing_gates[]`), **`count`**, **`universe_size`**, **`scan_generated_at`** (ISO UTC), **`engine_version`** (gate/rank contract id on the blob; Dynamo key is the stable `position_scan_snapshot`), **`cached`** (bool), **`disclaimer`**. **Gem** = hygiene (G8/G9/G3, not a sharp G5 breakdown) + F2 bullish + sector/news/geo bullish; mega-caps only as that same exception. **Strong** = G1-led quality compounder (including mega-caps the hunt skipped). G1–G9 still populate `failing_gates[]`. Transparent **`gem_rank`** = `0.45·fundamentals + 0.20·technical + 0.15·sector + 0.10·macro + 0.10·min(F1..F5)` (no LLM). Live scan = mid-cap discovery + curated mega board; weekly batch uses a larger discovery cap. Degrades to `{ candidates: [], degraded: true }` on scan failure (still HTTP 200). "Gem candidate" = informational screening, not a recommendation. BFF: **`GET /api/stocvest/signals/position/candidates`**.
