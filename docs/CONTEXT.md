...
**Test baseline (regression gate — must match §13):** Backend `pytest tests/ -q` → **356 passed**, **3 skipped**. Frontend `npm run test` → **46 passed** (17 test files). **`npm run build`** last verified: success.
...
|-------|---------|---------------|
| Backend | `pytest tests/ -q` | **356 passed**, **3 skipped** |
