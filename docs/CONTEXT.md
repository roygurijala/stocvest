...
**Test baseline (regression gate — must match §13):** Backend `pytest tests/ -q` → **906 passed**, **3 skipped**. Frontend `npm run test` → **161 passed** (30 test files). **`npm run build`** last verified: success. STOCVEST Assistant is now mounted in the **root layout** gated on session, and **every** dashboard page (`dashboard`, `dashboard/scanner`, `dashboard/signals` (+ history), `dashboard/watchlists`, `dashboard/signal-validation`, `dashboard/earnings`, `dashboard/portfolio`, `dashboard/journal`, `dashboard/performance`, `dashboard/options`, `dashboard/futures`, `dashboard/crypto`, `dashboard/settings`, `dashboard/legal`) calls `usePublishAssistantContext` so the chatbot is in contextual mode app-wide; logged-out marketing visitors continue to see CrispChat only.
...
|-------|---------|---------------|
| Backend | `pytest tests/ -q` | **906 passed**, **3 skipped** (verified 2026-05-10; +12 from prior 894/3 baseline — 11 new in `tests/signals/test_assistant_prompts.py` covering `serialize_page_context` whitelist + scanner fields, `_mode_from_context` page-alone contextual flip, and `sanitize_messages` guards; +1 new in `tests/api/handlers/test_signals_assistant_chat.py` covering `POST /v1/signals/assistant/chat` with a scanner-only `page_context`) |
