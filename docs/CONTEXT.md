...
**Test baseline (regression gate — must match §13):** Backend `pytest tests/ -q` → **885 passed**, **3 skipped**. Frontend `npm run test` → **164 passed** (31 test files). **`npm run build`** last verified: success.
...
|-------|---------|---------------|
| Backend | `pytest tests/ -q` | **885 passed**, **3 skipped** |
