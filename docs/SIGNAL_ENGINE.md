...
Gem discovery screens each candidate through `position_gem_gates.py` (G1–G9 still computed). **Gem tier** is growth-led: hygiene (G8/G9/G3, not a sharp G5 breakdown) + F2 bullish + **sector** bullish + **not** a mega-cap. News/geo are a catalyst flag, not membership. Mega-caps stay Strong/Monitor. The Invest/API gem board is the top **15** by `gem_rank`. Invest All/home is hunt-only (gems); mega-caps stay on Strong/Monitor. Invest row click expands why the name is on the board (F2 is latest-quarter YoY; a sector bid can be a cycle or geopolitical premium). Energy names stay on the list — Gem is a screen, not a hold-through-an-event call. **G8 — universe hygiene** delegates to **`position_universe_filter.py`** (`passes_position_universe_filter`). A failed G8 forces tier **Insufficient** (never surfaced), independent of fundamentals. The hunt pond is persisted (`position_scan_universe`) and re-scored on live refresh; weekly batch rebuilds it. Scan compose is scan-lite (no news pagination).
