...
5. **Scheduler / stale eval:** EventBridge Scheduler invokes the **scanner** Lambda (America/New_York, Mon–Fri):
   ...
   All call ``stocvest.workers.watchlist_maturation_refresh`` → ``sync_watchlist_maturation_from_composite``. Per-user symbol breadth follows **plan caps** in ``stocvest/api/services/watchlist_plan_limits.py``: **free 5**, **swing_pro 50**, **swing_day_pro 100** (beta full access → 100). Scheduled jobs scan up to **500** default lists per run with composite-call budgets (**2500** open slots, **1500** EOD) unless env overrides. **On demand:** watchlist row **Refresh** and **Evidence** use the same composite path. Scheduled runs pass ``email_on_state_change=False``.
