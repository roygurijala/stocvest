...
5. **Scheduler / stale eval:** EventBridge Scheduler invokes the **scanner** Lambda (America/New_York, Mon–Fri):
   - ``maturation_refresh_swing`` — **~8:15 AM** — swing composites only (after **8:00** laggard price warm).
   - ``maturation_refresh_day`` — **~9:35 AM** — day composites only; **skipped** when NYSE is not in regular session.
   - ``ledger_capture`` — **~3:55 PM** — validation **ledger gate capture** (day + swing composites inside ledger entry windows). Calls ``stocvest.workers.watchlist_ledger_capture`` with ``ledger_capture=True`` on composite engines — writes **qualified** or **shadow** rows to **`SignalHistory`** (see **B62** in ``docs/IMPLEMENTED.md``). Does **not** sync maturation state; prioritizes actionable + near-ready symbols; round-robin across users; env caps ``STOCVEST_LEDGER_CAPTURE_SCAN_LIMIT`` (default **500** watchlists) and ``STOCVEST_LEDGER_CAPTURE_MAX_CALLS`` (default **1500** composites/run).
   - ``maturation_refresh`` — **~4:30 PM** — EOD reconciliation (day + optional swing via ``STOCVEST_MATURATION_REFRESH_SWING``).
   Maturation refresh jobs call ``stocvest.workers.watchlist_maturation_refresh`` → ``sync_watchlist_maturation_from_composite``. Per-user symbol breadth follows **plan caps** in ``stocvest/api/services/watchlist_plan_limits.py``: **free 5**, **swing_pro 50**, **swing_day_pro 100** (beta full access → 100). Scheduled maturation jobs scan up to **500** default lists per run with composite-call budgets (**2500** open slots, **1500** EOD) unless env overrides. **On demand:** watchlist row **Refresh** and **Evidence** use the same composite path. Scheduled maturation runs pass ``email_on_state_change=False``.
