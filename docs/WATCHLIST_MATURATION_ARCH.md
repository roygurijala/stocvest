...
8. **Setup evolution log (2026-05-16, analytics B46 2026-05-17, score analytics B68 2026-06-10):** Table **`WatchlistMaturationTransition`** stores append-only rows when maturation **state** changes or **alignment** meaningfully changes (layer count, missing set, or bias). Keys: `pk` = `USER#{user_id}#SYM#{SYMBOL}#MODE#{mode}`, `sk` = `TS#{recorded_at_iso}`; GSI **`ModeTimelineIndex`** (`gsi1pk` = `MODE#swing|day`, `gsi1sk` = `{recorded_at}#{user_id}#{symbol}`) for platform aggregates. Optional **`price_at_event`** and **`signal_score`** (desk composite 0–100 from evidence composite when logged) on new rows. TTL **90 days**. APIs: per-symbol **`GET /v1/watchlists/symbols/{symbol}/setup-evolution`** (includes **`analytics`** score payload), user outcomes **`GET /v1/analytics/setup-outcomes`**, admin **`GET /v1/admin/system-behavior`**. UI: Signals/Trading Room **Evolution** tab, `/dashboard/setup-evolution`, `/dashboard/setup-outcomes` — see **`SETUP_ANALYTICS_SPEC.md`**.
