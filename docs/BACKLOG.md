...
**Last updated:** 2026-05-04 (signals stale-state fix + after-hours panel + dynamic layer reasoning)
...
| B15 | **Market Intelligence headlines (quality + affected stocks)** | Done 2026-05-04 | **Backend:** `news_handler` now pulls ticker-scoped market news (`ticker.any_of`, 4h lookback), merges default watchlist symbols (cap 30), filters PR-wire/noise via `news_quality_filter`, and adds `affected_stocks` + `impact_summary` via `news_impact_analyzer` (template-based, no AI call). **Frontend:** dashboard headlines renamed **Market Intelligence**, tiered source styling, sentiment label, affected-stock chips (impact/reason/watchlist star), chip click-through to `/dashboard/signals?symbol=...`, impact summary, and clean empty state. **Tests:** `tests/services/test_news_quality_filter.py`, `tests/services/test_news_impact_analyzer.py` plus handler/client coverage. |
