"""Write platform-level setup-evolution transitions."""
...
from stocvest.models.system_signal_transition import (
    SystemSignalTransition,
    derive_system_transition_type,
    should_log_system_transition,
)
...
def try_log_system_signal_transition(
    *,
    ...
    if not should_log_system_transition(prev, next_entry):
        return
    ...
    transition = SystemSignalTransition(
        ...
        transition_type=derive_system_transition_type(prev, next_entry),
    ...
    try:
        ...
        _LOG.warning(
            "system signal transition log failed sym=%s mode=%s: %s",
            next_entry.symbol,
            next_entry.mode,
            exc,
        )
