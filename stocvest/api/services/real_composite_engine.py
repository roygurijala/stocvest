    ...
    *,
    ...
    async with PolygonClient(api_key=settings.polygon_api_key) as client:
        bars_r, sym_r, news_r, spy_r, qqq_r, vix_r, econ_r = await asyncio.gather(
            ...
            client.get_market_news(tickers=[sym], limit=10),
    ...
    news = NewsAnalyzer().analyze(sym, news_rows, params.news)
    macro = MacroAnalyzer().analyze(spy_snap, qqq_snap, vix_snap, econ, params.macro)
    sector = SectorAnalyzer().analyze(sym, sector_snap, spy_snap, params.sector, sector_display_name=sector_display)
    geo = GeoAnalyzer().analyze(news_rows)
