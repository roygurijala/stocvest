"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import {
  fetchLiveSignals,
  fetchPerformanceSummary,
  formatHorizonOutcome,
  type PerformanceSummary,
  type PublicSignal
} from "@/lib/api/public-signals";

function outcomeLabel(outcome: PublicSignal["outcome"]): string {
  if (outcome === "win") return "✅ Direction correct";
  if (outcome === "loss") return "❌ Direction incorrect";
  if (outcome === "neutral") return "➖ Neutral";
  return "⏳ Pending";
}

interface PerformanceTrackingContentProps {
  showHomeLink?: boolean;
}

export function PerformanceTrackingContent({ showHomeLink = false }: PerformanceTrackingContentProps) {
  const [signals, setSignals] = useState<PublicSignal[]>([]);
  const [summary, setSummary] = useState<PerformanceSummary | null>(null);

  useEffect(() => {
    let active = true;
    const load = async () => {
      const [s, p] = await Promise.all([fetchLiveSignals(), fetchPerformanceSummary()]);
      if (!active) return;
      setSignals(s);
      setSummary(p);
    };
    void load();
    return () => {
      active = false;
    };
  }, []);

  const launchDate = summary?.launch_date ?? new Date().toISOString().slice(0, 10);
  const resolvedCount = summary?.signals_evaluated ?? 0;
  const showTable = signals.length > 0;

  return (
    <div className="overflow-x-hidden text-slate-100">
      {showHomeLink ? (
        <Link href="/" className="text-sm text-[#3b82f6] hover:underline">
          ← Back to home
        </Link>
      ) : null}
      <h1 className={`font-black text-slate-100 ${showHomeLink ? "mt-5 text-4xl md:text-6xl" : "text-2xl md:text-3xl"}`}>
        Signal Accuracy Tracking
      </h1>
      <p className={`max-w-4xl text-slate-300 ${showHomeLink ? "mt-3 md:text-xl" : "mt-2 text-sm md:text-base"}`}>
        Every signal generated by STOCVEST is tracked against subsequent market price movement. Directional accuracy reflects signal
        direction only — not trading returns. Individual results will vary based on entry timing, position size, and execution. Historical
        signal accuracy does not guarantee future results.
      </p>

      <section className={`grid grid-cols-2 gap-3 ${showHomeLink ? "mt-8 lg:grid-cols-4" : "mt-6 lg:grid-cols-4"}`}>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-400">Total Signals</p>
          <p className="mt-2 text-2xl font-bold">{summary?.total_signals_tracked ?? 0}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-400">Directional Accuracy</p>
          <p className="mt-2 text-2xl font-bold">
            {summary && summary.win_count + summary.loss_count > 0 ? `${summary.directional_accuracy_percent.toFixed(1)}%` : "—"}
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-400">Signals Evaluated</p>
          <p className="mt-2 text-2xl font-bold">{resolvedCount}</p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs uppercase tracking-wide text-slate-400">Tracking Since</p>
          <p className="mt-2 text-2xl font-bold">{launchDate}</p>
        </div>
      </section>

      {resolvedCount > 0 ? (
        <p className="mt-4 text-sm text-slate-400">
          Based on {resolvedCount} resolved signal{resolvedCount === 1 ? "" : "s"} since launch (1d horizon). Pending signals are excluded
          from the accuracy percentage.
        </p>
      ) : null}

      {!showTable ? (
        <section className={`rounded-xl border border-white/10 bg-white/5 p-6 ${showHomeLink ? "mt-8" : "mt-6"}`}>
          <h2 className="text-2xl font-bold">Accuracy Tracking Starts Now</h2>
          <p className="mt-3 text-slate-300">
            We launched on {launchDate}. Signal accuracy data accumulates as composite signals are recorded with symbol and price.
            <br />
            Every signal is evaluated for directional accuracy only (not dollar P&amp;L).
            <br />
            Check back after market hours once signals have been generated and resolved.
          </p>
          {showHomeLink ? (
            <Link
              href="/#live-signals"
              className="mt-5 inline-block rounded-md border border-white/20 px-4 py-2 text-sm hover:border-white/40"
            >
              View Live Signals
            </Link>
          ) : null}
        </section>
      ) : (
        <section className={`overflow-x-auto rounded-xl border border-white/10 ${showHomeLink ? "mt-8" : "mt-6"}`}>
          <table className="w-full min-w-[900px] bg-white/5 text-sm">
            <thead>
              <tr className="border-b border-white/10 text-left text-slate-300">
                <th className="px-4 py-3">Symbol</th>
                <th className="px-4 py-3">Direction</th>
                <th className="px-4 py-3">Signal Strength</th>
                <th className="px-4 py-3">Date and Time</th>
                <th className="px-4 py-3">1h</th>
                <th className="px-4 py-3">1d</th>
                <th className="px-4 py-3">Legacy</th>
              </tr>
            </thead>
            <tbody>
              {signals.map((row) => {
                const h1 = formatHorizonOutcome(row.outcome_1h);
                const h1d = formatHorizonOutcome(row.outcome_1d);
                return (
                  <tr key={row.signal_id ?? `${row.symbol}-${row.timestamp_iso}`} className="border-b border-white/10">
                    <td className="px-4 py-3 font-semibold">{row.symbol}</td>
                    <td className="px-4 py-3 capitalize">{row.bias}</td>
                    <td className="px-4 py-3">{Math.round(row.signal_strength)}%</td>
                    <td className="px-4 py-3 text-slate-300">{new Date(row.timestamp_iso).toLocaleString()}</td>
                    <td className="px-4 py-3 text-slate-300">{h1.label}</td>
                    <td className="px-4 py-3 text-slate-300">{h1d.label}</td>
                    <td className="px-4 py-3">{outcomeLabel(row.outcome)}</td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </section>
      )}

      <section className={`rounded-xl border border-white/10 bg-white/5 p-6 ${showHomeLink ? "mt-8" : "mt-6"}`}>
        <h2 className="text-2xl font-bold">Our commitment to transparency</h2>
        <ul className="mt-3 list-disc space-y-1 pl-6 text-slate-300">
          <li>We publish signal outcomes — not only directionally correct results</li>
          <li>Small sample sizes are shown with sample count</li>
          <li>Past signal accuracy does not guarantee future results</li>
          <li>Directional accuracy uses resolved 1d outcomes; correct ÷ (correct + incorrect), excluding neutral moves</li>
          <li>Moves within 0.1% are scored as neutral</li>
        </ul>
      </section>
    </div>
  );
}
