...
export function SignalLayerDivergenceChart({
  ...
  /** Reserve space so wrapped legend rows do not steal chart height from six layers. */
  const legendBlockPx = showLegend ? (isMobileLayout ? 56 : 44) : 0;
  const chartHeight = Math.max(isMobileLayout ? 200 : 176, height - legendBlockPx);
  const yAxisWidth = isMobileLayout ? 80 : 88;
  const tickSize = isMobileLayout ? 9 : 10;
  const chartMargins = isMobileLayout
    ? { top: 8, right: 4, left: 0, bottom: 12 }
    : { top: 4, right: 10, left: 2, bottom: 4 };
  ...
  return (
    <div className="w-full max-w-full min-w-0 touch-pan-y">
      {legend}
      <div className="w-full min-w-0 max-w-full min-h-0 overflow-x-auto overscroll-x-contain touch-pan-x">
        <div className="min-w-[280px]" style={{ height: chartHeight }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              ...
              layout="vertical"
              margin={chartMargins}
              barCategoryGap={isMobileLayout ? "14%" : "18%"}
            >
              <XAxis
                type="number"
              ...
              <Bar
                dataKey="delta"
                radius={[0, 2, 2, 0]}
                maxBarSize={isMobileLayout ? 18 : 16}
                minPointSize={8}
