...
// ─────────────────────────────────────────────────────────────────────────────
// (1) Hero strip — structural invariant
// ─────────────────────────────────────────────────────────────────────────────

describe("DashboardHeroStrip — structural invariants (Phase A1)", () => {
  test("hero_strip_renders_above_the_three_master_cards", () => {
    wrap(
      <DashboardRedesign
    ...
    const strip = screen.getByTestId("dashboard-hero-strip");
    expect(strip).toBeInTheDocument();
    const swing = screen.getByTestId("swing-desk-panel");
    // Hero strip must precede every master card in DOM order.
    expect(strip.compareDocumentPosition(swing) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  });

  test("hero_strip_carries_NO_data_card_role_so_the_three_master_cards_invariant_holds", () => {
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    // The hero strip and every cell inside it must NOT carry
    // `data-card-role`. Anything that does is counted as a master
    // card by `dashboard_has_exactly_three_master_cards_at_top_hierarchy_level`.
    const strip = screen.getByTestId("dashboard-hero-strip");
    expect(strip.hasAttribute("data-card-role")).toBe(false);
    const rolesInsideStrip = strip.querySelectorAll("[data-card-role]");
    expect(rolesInsideStrip.length).toBe(0);
  });

  test("hero_strip_uses_flex_wrap_so_it_collapses_gracefully_on_narrow_viewports", () => {
    // Mobile guard: at narrow widths the four cells must wrap, not
    // overflow horizontally. A regression that swaps to a fixed
    // grid-template-columns count would break mobile layout silently.
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    expect(style.toLowerCase()).toContain("flex-wrap: wrap");
    expect(style.toLowerCase()).toContain("display: flex");
  });

  test("hero_strip_renders_all_four_cells_with_stable_testids", () => {
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    expect(screen.getByTestId("hero-strip-regime")).toBeInTheDocument();
    expect(screen.getByTestId("hero-strip-vix")).toBeInTheDocument();
    expect(screen.getByTestId("hero-strip-breadth")).toBeInTheDocument();
    expect(screen.getByTestId("hero-strip-risk-horizon")).toBeInTheDocument();
  });
});

// ─────────────────────────────────────────────────────────────────────────────
// (2) Hero strip — projection of Shared Context
// ─────────────────────────────────────────────────────────────────────────────

describe("DashboardHeroStrip — projection of Shared Context (Phase A1)", () => {
  test("hero_strip_volatility_category_matches_shared_context_section_B", () => {
    // Same dataset → same category in both surfaces. Concretely:
    // the hero strip's `hero-strip-volatility-category` text MUST
    // contain the same category word ("Contained" / "Expanding" /
    // "Compressed" / "Unknown") that Section B's
    // `shared-context-volatility-category` shows. Drift here means
    // the dashboard tells the user two different things about the
    // same VIX number on the same page.
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    const heroVol = screen.getByTestId("hero-strip-volatility-category").textContent || "";
    const sharedVolCat =
      document.querySelector('[data-testid="shared-context-volatility-category"]')?.textContent || "";
    const categories = ["Contained", "Expanding", "Compressed", "Unknown"];
    for (const cat of categories) {
      const heroHas = heroVol.includes(cat);
      const sharedHas = sharedVolCat.includes(cat);
      // For each category word, the two surfaces agree (both yes OR both no).
      expect(heroHas).toBe(sharedHas);
    }
  });

  test("hero_strip_participation_category_matches_shared_context_section_C", () => {
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    const heroPart = screen.getByTestId("hero-strip-participation-category").textContent || "";
    const sharedPart =
      document.querySelector('[data-testid="shared-context-participation-category"]')?.textContent || "";
    const categories = ["Broad", "Mixed", "Narrow", "Unknown"];
    for (const cat of categories) {
      expect(heroPart.includes(cat)).toBe(sharedPart.includes(cat));
    }
  });

  test("hero_strip_breadth_dot_count_equals_sector_rotation_input_count", () => {
    wrap(
      <DashboardRedesign
        marketOverview={baseMarket}
        scannerOverview={baseScanner}
    ...
    for (const s of baseSectors) {
      const dot = screen.getByTestId(`hero-strip-breadth-dot-${s.symbol}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// (3) Chatbot contract — payload invariance (Phase A1 + A2 must NOT change it)
// ─────────────────────────────────────────────────────────────────────────────
