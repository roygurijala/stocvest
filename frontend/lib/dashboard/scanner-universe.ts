...
export function symbolsFromDeskSlice(
  ...
  opts?: { discoveryLimit?: number; moversLimit?: number }
): string[] {
  if (!data) return [];
...
/**
 * Cap universe while preserving priority symbols first (anchors → desk → gap → watchlist).
 */
export function capScannerUniverse(universe: string[], max: number, priority: string[]): string[] {
