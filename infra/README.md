...
- **EventBridge (D1):** `infra/eventbridge.tf` — rule **`stocvest-signal-resolution`** (`rate(30 minutes)`) → **`signal_resolution`** Lambda (1h/1d outcomes). Terraform **ready**; requires **`terraform apply`** in AWS (see [`docs/D1_SIGNAL_RESOLUTION_SCHEDULE.md`](../docs/D1_SIGNAL_RESOLUTION_SCHEDULE.md)).
