...
- **EventBridge (D1):** `infra/eventbridge.tf` — rule **`stocvest-signal-resolution`** (`rate(30 minutes)`) → **`signal_resolution`** Lambda (1h/1d outcomes). **AWS (2026-05-03):** rule verified **ENABLED** via CLI; a full **`terraform plan -var-file=terraform.tfvars`** can still show a **large** diff (e.g. 19/14/2) if state lags repo — review plan before **`terraform apply`**. See [`docs/D1_SIGNAL_RESOLUTION_SCHEDULE.md`](../docs/D1_SIGNAL_RESOLUTION_SCHEDULE.md).
