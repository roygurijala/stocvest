...
def _fake_run_close_coro(coro: Any, result: dict[str, int]) -> dict[str, int]:
...
def test_handler_accepts_eventbridge_event(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_run(coro: Any) -> dict[str, int]:
        return _fake_run_close_coro(coro, {"resolved_1h": 2, "resolved_24h": 0})
...
def test_handler_logs_resolution_counts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(coro: Any) -> dict[str, int]:
        return _fake_run_close_coro(coro, {"resolved_1h": 3, "resolved_24h": 1})
