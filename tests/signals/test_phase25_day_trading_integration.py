...
from stocvest.signals.day_trading_scanner import IntradaySetupScanner, PremarketGapScanner, SymbolLiquidityContext
...
def test_phase25_pre_market_to_briefing_pipeline() -> None:
    ...
    base = datetime(2026, 4, 28, 9, 30, tzinfo=et)
    bars_by_symbol = {
        "GAP1": _opening_block(base, "GAP1")
        + [
            _bar("GAP1", 100.2, base + timedelta(minutes=15), vol=100_000),
            _bar("GAP1", 102.2, base + timedelta(minutes=16), vol=400_000),
        ]
    }
    liq = {"GAP1": SymbolLiquidityContext(avg_daily_volume=8_000_000.0, last_price=102.0, company_name=None)}
    setups = IntradaySetupScanner(min_score=0.5).scan(bars_by_symbol, liquidity_by_symbol=liq, limit=5)
    assert setups
